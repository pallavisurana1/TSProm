...
cd /data/private/psurana/TSProm/src/3_attention/ 
PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:35000
...
python 1_raw_attention_extract-gpu.py --gpu 6 --batch-size 64
...
if __name__ == "__main__":
    ...
    print(f"📋 Processing {len(jobs)} jobs with batch size {BATCH_SIZE}")
    ...
    for i, job in enumerate(jobs, 1):
        try:
            print(f"\n{'='*60}")
            print(f"🔄 Job {i}/{len(jobs)}: {job['base_model_nm']} | {job['subset']} | {job['lr_dir']}")
            print(f"{'='*60}")
            ...
            print(f"❌ Error in job {job['base_model_nm']} | {job['subset']}: {e}")
            failed_jobs += 1
